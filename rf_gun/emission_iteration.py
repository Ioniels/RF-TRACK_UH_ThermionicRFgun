...
from .cathode_fields import extraction_field, sample_rf_field_on_cathode, extract_sc_and_mirror_from_snapshot
...
def run_emission_field_iteration(
    ...
    """Under-relaxed Picard iteration coupling emission current to E_SC+E_mirror at the cathode
    (guide Sec. 7-8). Stage A (iteration 0: RF-only baseline, no SC/mirror update yet) and Stage B
    (iteration 1: first update) fall out naturally as the first two passes of the same loop that
    implements Stage C (fixed-point iteration to convergence); Stage D (beam loading) is omitted
    per config.include_beam_loading, which this repository's RF-Track binding cannot currently
    satisfy (see rf_gun.cathode_fields.extract_beam_loading_field).
    """
    ...
    emission_law = str(getattr(emission_params, "emission_law", "RD_schottky"))
    ...
    J_hist, E_rf_hist, E_sc_hist, E_mirror_hist, E_tot_hist = [], [], [], [], []
    ...
    for k in range(int(config.max_iterations)):
        ...
        E_total_new = E_RF + E_sc_new + E_mirror_new
        ...
        J, N_i = J_new, N_i_new
        E_sc, E_mirror = E_sc_new, E_mirror_new
        ...
        if on_iteration is not None:
            partial = EmissionFieldIterationResult(
                ...
                E_mirror_history_Vpm=E_mirror_hist, E_total_history_Vpm=E_tot_hist,
